...
W2G_URL = 'https://w2g.tv/rooms/create.json'
...
def make_url_string():
  return f"https://w2g.tv/rooms/{CURRENT_ROOM_KEY}"

def create_room(yt_url):
  post_req = {"w2g_api_key": W2G_API_KEY, "share": yt_url, "bg_color": "#2d3c4a", "bg_opacity": "50"}
...
def add_to_playlist(yt_url):
  ...
  json_post = json.dumps(post_req, indent = 4)
  res = requests.post(f"https://w2g.tv/rooms/{CURRENT_ROOM_KEY}/playlists/current/playlist_items/sync_update", data = json_post, headers = W2G_HEADERS)
...
async def room(ctx):
  ...
  if CURRENT_ROOM_KEY != "":
    ...
    room_msg = await ctx.channel.send("no current room found. create one with: $w2g url")
